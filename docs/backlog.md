...
- Acceptance:
  - Pressing "Configure filters" opens a full list of filters with selected ones marked.
  - Users can toggle multiple filters and save; previous screen updates accordingly.
  - Text/labels follow existing i18n policy.

### NEW) Admin: password-based login (no manual header)
- Goal: Replace manual Authorization header with a simple password-based login for the admin UI.
- Scope: Add login page (form) and session/cookie-based auth; keep token option as fallback. Minimal rate limiting.
- Acceptance:
  - Admin UI accessible via login form (username/password or password-only) when `ADMIN_ENABLED=true`.
  - Successful login establishes a session; manual header input is no longer needed for UI.

### NEW) Admin: protect created_at/updated_at (server-managed only)
- Goal: Prevent manual edits of audit timestamps in all admin forms.
- Scope: Make `created_at`/`updated_at` read-only or hidden in admin; enforce server-side defaults and automatic updates.
- Acceptance:
  - Admin UI does not allow setting these fields manually.
  - DB-level/default logic ensures values are generated/updated automatically.

### NEW) Admin: CRUD for Monsters and Spells
- Goal: Allow admin users to create, edit, and delete monsters and spells via the admin UI.
- Scope: Add SQLAdmin views/forms with validation for key fields; preserve existing i18n policy.
- Acceptance:
  - Admin can update core fields for monsters/spells; changes persist.
  - Validation errors are shown clearly; non-editable derived fields remain protected.

### NEW) Telegram: minimal admin editing for typos
- Goal: Allow users with `is_admin` flag to quickly fix typos in names/descriptions via Telegram.
- Scope: Bot commands/flows gated by `is_admin` to edit specific fields (e.g., title/description) with audit.
- Acceptance:
  - Admin users can issue a command to fetch and edit a field; changes saved via API.
  - Non-admin users are denied; actions are logged (who/what/when).

### NEW) Wait for Postgres readiness before running Alembic in prod compose
- Goal: Eliminate startup log noise and transient failures when API runs Alembic before Postgres is ready.
- Scope: Update production compose command generated by `scripts/deploy.sh` to wait on DB readiness (e.g., `pg_isready`) before `alembic upgrade head`.
- Acceptance:
  - API startup command blocks until Postgres accepts connections.
